...
- **Skill Tree** — an alternate view of the Sound Drills tab (toggle with the List/Skill Tree switch above the drill list). The 8 original categories are always-unlocked "Foundations"; the 8 newer categories are "Advanced" and unlock once their paired Foundation category is fully completed once. Every category that's been completed at least once earns 0–3 mastery stars based on recorded mic-attempt accuracy for that sound.
- **Daily Weak-Spot Challenge** — a banner at the top of the Drills tab offering a 5-item mixed set (drill words + shadowing sentences) picked from the learner's most-flagged "focus areas." The same 5 picks persist across reloads for a calendar day, then refresh the next day. New learners with no miss history yet get a sensible fallback set instead (least-completed content) so the challenge is useful from day one.
